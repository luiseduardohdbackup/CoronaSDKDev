...
function scene:createScene( event )
	...
	self.board:writeText(17,10,"A Production of",asciiBoardCell.createCell(0,colors.lightGreen,colors.black))
	self.board:writeText(18,17,"PlayDeez Games",asciiBoardCell.createCell(0,colors.lightGreen,colors.black))

	self.board:render(self.grid,self.gameData.resources.colors)
end

function scene:onKeyDown(theKey)
	timer.cancel(self.splashTimer)
...
function scene:willEnterScene( event )
    local group = self.view
	self.splashTimer = timer.performWithDelay(2000,self)
end
